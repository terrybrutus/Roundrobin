...
export default function RoundRobinApp() {
  ...
  const addBet = (bet: Bet) => {
    ...
    const categoryCount = selectedBets.filter((b) => getCategory(b.odds) === category).length;
    const maxForCategory = ODD_CATEGORIES[category as keyof typeof ODD_CATEGORIES].count;

    if (categoryCount >= maxForCategory) {
      setError(`Maximum ${maxForCategory} bets for ${ODD_CATEGORIES[category as keyof typeof ODD_CATEGORIES].label} reached`);
      return;
    }

    setSelectedBets([...selectedBets, { ...bet, id: Math.random().toString() }]);
    setError(null);
  };
  ...
  const structureCounts = {
    minus200: selectedBets.filter((b) => getCategory(b.odds) === "minus200").length,
    minus300: selectedBets.filter((b) => getCategory(b.odds) === "minus300").length,
    minus500: selectedBets.filter((b) => getCategory(b.odds) === "minus500").length,
    plus100: selectedBets.filter((b) => getCategory(b.odds) === "plus100").length,
  };
          ...
          <>
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
              <div className="lg:col-span-2">
                <OddsExplorer sports={sports} onAddBet={addBet} apiKey={apiKey} />
              </div>
